...
import { 
  ...
  Info, TrendingUp, AlertTriangle, ShieldCheck 
} from 'lucide-react';
...
export function TradingChart({ pipeline, onSelectConcept, replayIndex }: TradingChartProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const [dimensions, setDimensions] = useState({ width: 800, height: 500 });
  const [viewOffset, setViewOffset] = useState(0); // Pan offset
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#0b101d] rounded-xl border border-[#1e293b] overflow-hidden select-none">
      {/* Top Toolbar */}
      <div className="flex flex-wrap items-center justify-between px-4 py-2.5 bg-[#0e1628] border-b border-[#1e293b] gap-2">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono font-bold text-emerald-400 text-sm">
              {pipeline.lastPrice.toFixed(pipeline.instrument.assetClass === 'forex' ? 5 : 2)}
            </span>
          </div>
        </div>
      ...
      >
        ...
        >
          <defs>
            {/* Gradients for Premium / Discount Zones */}
            ...
            return (
              <g key={`grid-h-${i}`}>
                ...
                <text x={chartWidth + 6} y={y + 3} fill="#64748b" fontSize="10" fontFamily="monospace">
                  {priceVal.toFixed(pipeline.instrument.assetClass === 'forex' ? 4 : 1)}
                </text>
              </g>
            );
          })}
          ...
          {layers.showDealingRange && pipeline.dealingRange && (
            <g className="cursor-pointer" onClick={() => onSelectConcept?.('premium_discount')}>
              ...
              <text x={chartWidth - 110} y={priceToY(pipeline.dealingRange.equilibrium) - 4} fill="#38bdf8" fontSize="10" fontWeight="bold">
                EQ 50% ({pipeline.dealingRange.equilibrium.toFixed(pipeline.instrument.assetClass === 'forex' ? 4 : 2)})
              </text>

              {/* Discount Zone (Range Low to 50%) */}
              <rect
                x="0"
            ...
            return (
              ...
              >
                ...
                <text
                  ...
                  fontWeight="bold"
                >
                  {pool.type} ({pool.price.toFixed(pipeline.instrument.assetClass === 'forex' ? 4 : 2)}) {isSwept ? '⚡ SWEPT' : 'POOL'}
                </text>
              </g>
            );
          })}
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-[#090e1a] border-t border-[#1e293b] text-xs text-slate-400">
        <div className="flex items-center gap-4">
          <span className="flex items-center gap-1.5">
            ...
            <strong className="text-slate-200">Active Pipeline:</strong> Deterministic SMC Mathematical Core
          </span>
