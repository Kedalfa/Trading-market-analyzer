...
import { DEFAULT_RISK_SETTINGS, calculatePositionSize, createTradeIdeaFromAnalysis } from '@/services/riskService';
import { RiskSettings, TradeIdea } from '@/types/trade';
import { 
  X, Shield, Calculator, CheckCircle2, AlertTriangle, 
  Copy, Download, Send, ArrowRight, DollarSign, Target 
} from 'lucide-react';
...
export function RiskCalculatorModal({
  ...
  // Recalculate default values when direction changes
  useEffect(() => {
  ...
  const handleSendToJournal = () => {
    // Save to local storage for journal integration
    const existing = JSON.parse(localStorage.getItem('smc_trade_ideas') || '[]');
    existing.unshift(tradeIdea);
    localStorage.setItem('smc_trade_ideas', JSON.stringify(existing));
    setExported(true);
        ...
        <div className="flex items-center justify-between p-5 border-b border-[#1e293b] bg-[#0e1628]">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Position sizing and risk management integration</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto space-y-6 text-xs text-slate-300">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
            ...
            >
              ...
              {copied ? 'Copied to Clipboard!' : 'Copy Trade Idea JSON'}
            </button>

            <button
              onClick={handleSendToJournal}
              className="flex items-center gap-1.5 px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold shadow-lg shadow-emerald-600/30 transition-all"
            >
              <Send className="w-3.5 h-3.5" />
              {exported ? 'Exported to Journal!' : 'Export to Trading Journal Platform'}
            </button>
          </div>
        </div>
      </div>
