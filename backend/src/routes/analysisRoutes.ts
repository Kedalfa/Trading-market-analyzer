...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    if (timeframe) filter.timeframe = String(timeframe);
    if (htfBias) filter.htfBias = String(htfBias).toUpperCase();
    if (grade) filter['setupQuality.grade'] = String(grade).toUpperCase();
 ...
 */
router.patch('/:analysisId/outcome', async (req: Request, res: Response) => {
  try {
