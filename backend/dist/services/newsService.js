...
exports.getEventsForInstrument = getEventsForInstrument;
const node_cache_1 = __importDefault(require("node-cache"));
const config_1 = require("../config/config");
const cache = new node_cache_1.default({ stdTTL: config_1.config.newsCacheTtl });
/**
 * Currency → affected instruments map
 */
const CURRENCY_INSTRUMENT_MAP = {
 ...
 */
function getEventsForInstrument(instrumentId) {
    const cacheKey = `news:${instrumentId}`;
    const cached = cache.get(cacheKey);
    if (cached)
        return cached;
    const all = buildDynamicCalendar().filter((ev) => ev.affectedInstruments.includes(instrumentId));
